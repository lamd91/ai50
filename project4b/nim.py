...
class NimAI():
    ...
    def get_q_value(self, state, action):
        ...
        """
        # if (tuple(state), action) in self.q else 0
        ...
        if len(available_actions) == 0:
            ...
            for action in available_actions:
                if self.get_q_value(state, action) > best_future_reward:
                    best_future_reward = self.get_q_value(state, action)
    ...
    def choose_action(self, state, epsilon=True):
        ...
        if random.random() <= self.epsilon:
            ...
            highest_q_value_sofar = float('-inf')
            best_action = tuple()
            for action in tuple(available_actions):
                if self.get_q_value(tuple(state), action) >= highest_q_value_sofar:
                    highest_q_value_sofar = self.get_q_value(tuple(state), action)
                    best_action = action
